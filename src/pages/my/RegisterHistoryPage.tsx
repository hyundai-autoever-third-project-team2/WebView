...
function RegisterHistoryPage() {
    ...
    // 리액트 쿼리 데이터 패칭하기
    const { data: transactions = [], isLoading, error } = useQuery<CarViewUserCarTransactionData[], Error>({
      queryKey: ['userCarTransactions'],
      queryFn: fetchViewUserCarTransactionList,
 ...
 const filteredTransactions = transactions.filter(transaction => {
    ...
    const matchesYear = !selectedYear || 
        new Date(transaction.purchase_date).getFullYear().toString() === selectedYear;

    // Progress 필터
    const matchesProgress = !selectedProgress || 
        transaction.progress === selectedProgress;
    ...
    const handleStatClick = (label: string) => {
        ...
        setSelectedProgress(prevStatus => 
